...
type Input = {
    ...
    setNoticiaId: (value: number | null) => void;
    setIsOpenModal: (value: boolean) => void;
    onSuccess: () => Promise<void>;
}

const ModalNoticias = ({ onSuccess, isOpen, noticiaId, setIsOpenModal, setNoticiaId }: Input) => {
    ...
    const onSubmit = async () => {
        ...
        let isInvalid = false;
        if (titulo.length < 1) {
            toast.warn('Título da Noticia não pode ser vazio', { position: 'top-left' })
            isInvalid = true;
        }
        ...
        if (dt_inicio > dt_fim) {
            toast.warn('Data de inicio não pode ser maior que a data final', { position: 'top-left' })
            isInvalid = true;
        }
        ...
        if (noticiaId) {
            formData.append("cod", noticiaId.toString());
        }


        const result = await http.post("/noticias/", formData, {
            headers: {
        ...
        if (result.success) {
            if (isEditModal) {
                toast.success('Notícia editada com sucesso!', { position: 'top-left' });
            } else {
                toast.success('Notícia cadastrada com sucesso!', { position: 'top-left' });
            }
    ...
    const onLoadBanner = async () => {
        ...
        if (noticiaId) {
            const query = await http.get('/noticias/' + noticiaId)
                .then(e => ({ data: e.data, success: true, error: null, message: 'OK' }))
                .catch(e => ({ data: null, success: false, error: e.response?.data, message: e.response?.data || e.message }));
    ...
    return (
        <Drawer onClose={onCloseModal} closeOnOverlayClick={false} closeOnEsc={false} isOpen={isOpen} size={'md'}>
            ...
            <DrawerContent>
                <DrawerCloseButton />
                <DrawerHeader as='p'>{noticiaId ? 'Editar Notícia' : 'Novo Notícia'}</DrawerHeader>
                <DrawerBody>
                    {isLoading ? <Loading /> :
