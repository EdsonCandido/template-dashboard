...
type dataFormat = Noticia & { index: number };
...
const ListNoticias = ({ isLoadingPage, dataFormat, onPressModal, onSuccess }: Input) => {
    ...
    const handleAtivo = async (cod: number, is_ativo: number) => {
        const result = await http.delete("noticias/" + cod)
        ...
        if (result.success) {
            toast.success(`Notícia ${is_ativo === 1 ? 'desativada' : 'ativada'} com sucesso`);
            await onSuccess();
        } else {
            ...
            isLoadingPage ?
                ...
                : <Stack w={'100%'} bg={'white'} borderRadius={'5px'} p={'10px'} gap={'10px'} boxShadow={'lg'} >
                    <DataGrid
                        ...
                        <Column caption="Criada em." dataField="created_at" minWidth={100} cellRender={(e) => <Text> {dateFormat(e.value, 'dd/MM/yyyy')}</Text>} />
