...
const ListBanners = ({ isLoadingPage, dataFormat, onPressModal, onSuccess }: Input) => {
    ...
    const handleAtivo = async (cod: number, is_ativo: number) => {
        const result = await http.delete("banners/" + cod)
        ...
        if (result.success) {
            toast.success(`Banner ${is_ativo === 1 ? 'desativado' : 'ativado'} com sucesso`);
            await onSuccess();
        } else {
